...
/obj/item/pet_carrier/container_resist(mob/living/user)
	...
	if(user.mob_size <= MOB_SIZE_SMALL)
		...
		if(!do_after(user, rand(300, 400), target = user) || open || !locked || !user in occupants)
			return
		loc.visible_message("<span class='warning'>[user] flips the lock switch on [src] by reaching through!</span>", user)
		to_chat(user, "<span class='boldannounce'>Bingo! The lock pops open!</span>")
		locked = FALSE
		playsound(src, 'sound/machines/boltsup.ogg', 30, TRUE)
		update_icon()
	else
		loc.visible_message("<span class='warning'>[src] starts rattling as something pushes against the door!</span>", user)
		...
		if(!do_after(user, 200, target = user) || open || !locked || !user in occupants)
			return
		loc.visible_message("<span class='warning'>[user] shoves out of	[src]!</span>", user)
...
/obj/item/pet_carrier/proc/load_occupant(mob/living/user, mob/living/target)
	...
	"<span class='notice'>You start loading [target] into [src]...</span>", target)
	...
	"<span class='notice'>You load [target] into [src].</span>", target)
